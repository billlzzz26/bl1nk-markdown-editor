...
def run_loop(
    ...
    resolved_optimizer_platform = resolve_optimizer_platform(
        optimizer_platform, platform
    )
...
def main():
    ...
    parser.add_argument(
        "--model",
        default=None,
        help="Model to use where the selected runtime supports it",
    )
    parser.add_argument(
    ...
    if args.report != "none":
        if args.report == "auto":
            ...
            # Validate the resolved path stays within the specified parent directory
            allowed_parent = Path(args.report).resolve().parent
            if not str(live_report_path).startswith(str(allowed_parent)):
                print(f"Error: report path traversal detected", file=sys.stderr)
                sys.exit(1)
